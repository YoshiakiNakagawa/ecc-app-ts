...
const SetSizesArea = (props:any) => {
    ...
    const inputSize = useCallback((event:any) => {
        setSize(event.target.value)
    }, [setSize]);

    const inputQuantity = useCallback((event:any) => {
    ...
    const deleteSize = (deleteIndex:number) => {
        const newSizes = props.sizes.filter((item:any, index:number) => index !== deleteIndex)
        props.setSizes(newSizes);
    }
